-- Migration: fix_all_search_path_security_issues
-- Description: Correction complète de toutes les vulnérabilités search_path détectées par Supabase Advisors
-- Date: 2025-08-06
-- Problème: Les fonctions avec SECURITY DEFINER doivent avoir un search_path fixe pour éviter les attaques par injection

-- ============================================================================
-- CORRECTION DE TOUTES LES FONCTIONS AVEC PROBLÈME DE SEARCH_PATH
-- ============================================================================

-- 1. calculate_stage_actual_duration (MANQUANTE - À CRÉER)
...
-- 6. get_stages_requiring_attention
...
-- ============================================================================
-- CRÉATION DU TRIGGER POUR calculate_stage_actual_duration
-- ============================================================================

-- Supprimer le trigger existant s'il existe
DROP TRIGGER IF EXISTS trg_calculate_stage_actual_duration ON public.folder_processing_stages;

-- Créer le trigger
CREATE TRIGGER trg_calculate_stage_actual_duration
...
COMMENT ON FUNCTION calculate_stage_actual_duration() IS 'Trigger function pour calculer automatiquement la durée réelle des étapes - SÉCURISÉ';
COMMENT ON FUNCTION user_can_access_folder(uuid, uuid) IS 'Vérifie si un utilisateur peut accéder à un dossier donné - SÉCURISÉ';
COMMENT ON FUNCTION initialize_folder_stages(uuid, uuid) IS 'Initialise toutes les étapes de traitement pour un nouveau dossier - SÉCURISÉ';
COMMENT ON FUNCTION start_processing_stage(uuid, public.processing_stage_enum, uuid, uuid, text) IS 'Démarre une étape spécifique - SÉCURISÉ';
COMMENT ON FUNCTION get_folder_progress(uuid) IS 'Récupère les statistiques de progression d''un dossier - SÉCURISÉ';
COMMENT ON FUNCTION get_stages_requiring_attention() IS 'Récupère les étapes nécessitant une attention urgente - SÉCURISÉ';
COMMENT ON FUNCTION complete_processing_stage(uuid, public.processing_stage_enum, uuid, text, text[]) IS 'Marque une étape comme complétée - SÉCURISÉ';
COMMENT ON FUNCTION block_processing_stage(uuid, public.processing_stage_enum, text, uuid) IS 'Bloque une étape avec raison - SÉCURISÉ';
COMMENT ON FUNCTION unblock_processing_stage(uuid, public.processing_stage_enum, uuid, text) IS 'Débloque une étape - SÉCURISÉ';
COMMENT ON FUNCTION approve_processing_stage(uuid, public.processing_stage_enum, uuid, text) IS 'Approuve une étape nécessitant validation - SÉCURISÉ';
COMMENT ON FUNCTION skip_processing_stage(uuid, public.processing_stage_enum, uuid, text) IS 'Ignore une étape optionnelle - SÉCURISÉ';
COMMENT ON FUNCTION get_stage_performance_metrics() IS 'Récupère les métriques de performance par étape - SÉCURISÉ';
COMMENT ON FUNCTION user_can_modify_stage(uuid, uuid) IS 'Vérifie si un utilisateur peut modifier une étape de traitement donnée - SÉCURISÉ';
COMMENT ON FUNCTION user_can_approve_stage(uuid, uuid) IS 'Vérifie si un utilisateur peut approuver une étape de traitement donnée - SÉCURISÉ';

-- Message de confirmation
DO $$
BEGIN
    RAISE NOTICE '================================================================';
    RAISE NOTICE 'MIGRATION COMPLÉTÉE: Toutes les fonctions search_path corrigées';
    RAISE NOTICE '================================================================';
    RAISE NOTICE '✅ 14 fonctions sécurisées avec SET search_path = ''public''';
    RAISE NOTICE '✅ Fonction calculate_stage_actual_duration créée avec trigger';
    RAISE NOTICE '✅ Toutes les vulnérabilités search_path résolues';
    RAISE NOTICE '================================================================';
END $$;