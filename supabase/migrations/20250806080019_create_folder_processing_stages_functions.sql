-- Migration: create_folder_processing_stages_functions
-- Description: Fonctions de gestion des transitions et workflow des étapes de traitement
-- Date: 2025-08-06

-- ============================================================================
-- Fonctions de gestion du workflow des étapes
-- ============================================================================

-- Fonction pour initialiser toutes les étapes d'un dossier
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM folders WHERE id = p_folder_id AND deleted_at IS NULL) THEN
    RAISE EXCEPTION 'Folder with ID % not found or deleted', p_folder_id;
  END IF;

  -- Supprimer les étapes existantes si elles existent
  DELETE FROM folder_processing_stages WHERE folder_id = p_folder_id;

  -- Créer toutes les étapes basées sur la configuration par défaut
  FOR stage_config IN 
    SELECT * FROM default_processing_stages ORDER BY sequence_order
  LOOP
    INSERT INTO folder_processing_stages (
      folder_id,
      stage,
      status,
      sequence_order,
      ...
      p_folder_id,
      stage_config.stage,
      CASE WHEN stage_config.sequence_order = 1 THEN 'pending'::stage_status_enum ELSE 'pending'::stage_status_enum END,
...
-- Fonction pour démarrer une étape spécifique
CREATE OR REPLACE FUNCTION start_processing_stage(
  p_folder_id uuid,
  p_stage processing_stage_enum,
...
AS $$
DECLARE
  current_status stage_status_enum;
BEGIN
  ...
  UPDATE folder_processing_stages
  SET 
    status = 'in_progress',
    started_at = now(),
...
-- Fonction pour bloquer une étape
...
-- Fonction pour approuver une étape nécessitant une approbation
...
-- Fonction pour ignorer une étape (si autorisé)
...
-- ============================================================================
-- Fonctions de requête et d'analyse
-- ============================================================================

-- Fonction pour obtenir la progression d'un dossier
...
RETURNS TABLE (
  ...
  current_stage processing_stage_enum,
  next_stage processing_stage_enum,
...
-- Fonction pour obtenir les étapes nécessitant attention
CREATE OR REPLACE FUNCTION get_stages_requiring_attention()
RETURNS TABLE (
  folder_id uuid,
  folder_number varchar,
  stage processing_stage_enum,
  status stage_status_enum,
  assigned_to uuid,
  assignee_name text,
  days_overdue integer,
  priority stage_priority_enum,
...
-- Fonction pour obtenir les performances par étape
...
GRANT EXECUTE ON FUNCTION initialize_folder_stages(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION start_processing_stage(uuid, processing_stage_enum, uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION complete_processing_stage(uuid, processing_stage_enum, uuid, text, text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION block_processing_stage(uuid, processing_stage_enum, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION unblock_processing_stage(uuid, processing_stage_enum, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION approve_processing_stage(uuid, processing_stage_enum, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION skip_processing_stage(uuid, processing_stage_enum, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION get_folder_progress(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION get_stages_requiring_attention() TO authenticated;
GRANT EXECUTE ON FUNCTION get_stage_performance_metrics() TO authenticated;

-- ============================================================================
-- Commentaires pour documentation
-- ============================================================================

COMMENT ON FUNCTION initialize_folder_stages(uuid, uuid) IS 'Initialise toutes les étapes de traitement pour un nouveau dossier';
COMMENT ON FUNCTION start_processing_stage(uuid, processing_stage_enum, uuid, uuid, text) IS 'Démarre une étape spécifique';
COMMENT ON FUNCTION complete_processing_stage(uuid, processing_stage_enum, uuid, text, text[]) IS 'Marque une étape comme complétée';
COMMENT ON FUNCTION block_processing_stage(uuid, processing_stage_enum, text, uuid) IS 'Bloque une étape avec une raison';
COMMENT ON FUNCTION unblock_processing_stage(uuid, processing_stage_enum, uuid, text) IS 'Débloque une étape et restaure son statut';
COMMENT ON FUNCTION approve_processing_stage(uuid, processing_stage_enum, uuid, text) IS 'Approuve une étape nécessitant validation';
COMMENT ON FUNCTION skip_processing_stage(uuid, processing_stage_enum, uuid, text) IS 'Ignore une étape optionnelle';
COMMENT ON FUNCTION get_folder_progress(uuid) IS 'Calcule la progression complète d''un dossier';
COMMENT ON FUNCTION get_stages_requiring_attention() IS 'Identifie les étapes nécessitant une attention urgente';
COMMENT ON FUNCTION get_stage_performance_metrics() IS 'Calcule les métriques de performance par type d''étape';